Public Class SellingUI_AlphaNumKeyBoard_Client
    ...
    Private Sub NumericKeyBoard_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Me.ClientTableAdapter.Fill(Me.PharmacyDataSet.Client)
        myString = ""
    ...
    Private Sub displayVal()
        ...
        cmbClientName.Text = myString.Trim
        'lbVal.Text = myString
        GroupBox1.Focus()
    End Sub
    ...
    Private Sub but_OK_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles but_OK.Click
        ...
        Dim idClient = cmbClientName.SelectedValue
        If (idClient <= 0) Then idClient = insertClient()
        If (idClient <= 0) Then idClient = 0
        cmbClientName.Text = ""
        ' MsgBox(idClient)
        If (Not IsNothing(myTextBox)) Then
            myTextBox.Text = idClient
            myForm = Nothing
            myTextBox = Nothing
    ...
    Private Function insertClient() As Integer
        Dim clientName = cmbClientName.Text.Trim
        If clientName = "" Then Return 0
        Dim idClient = cmbClientName.SelectedValue
        Dim address = ""
        Dim tel = ""
        Dim comment = ""
        If idClient <= 0 Then
            Dim idC = ClientTableAdapter.InsertRow(clientName, tel, address, comment)
            LogTool.log("SellingInterface_Mobile", "insertClient", "Client", "InsertRow", clientName & ":" & tel & ":" & address & ":" & comment, Date.Now, idC)
            ClientTableAdapter.Fill(Me.PharmacyDataSet.Client)
            cmbClientName.Text = clientName
        End If

        idClient = cmbClientName.SelectedValue
        Return idClient
    End Function

    Private Sub cmbClientName_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles cmbClientName.SelectedIndexChanged
        lbVal.Text = "ID:" & cmbClientName.SelectedValue
    End Sub
